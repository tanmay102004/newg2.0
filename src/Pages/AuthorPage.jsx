...
import { getResolvedAuthorPageContent } from '../data/articlePages'
...
function AuthorPage({ content, query }) {
  const authorName = query.get('author')?.trim() ?? ''
  const section = getResolvedAuthorPageContent(authorName, content)
  ...
  return (
    <main className="news-page breaking-page-shell" lang="hi">
      ...
      <div className="breaking-page author-page">
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            <header className="breaking-page__header author-page__header">
              <h1>{section.title}</h1>
            ...
            <nav className="breaking-page__pagination" aria-label="Author page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?author=${encodeURIComponent(authorName)}&page=${pageNumber}`}
