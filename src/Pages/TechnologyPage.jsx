...
import { defaultTechnologyPage } from '../data/technologyPage'
...
function TechnologyPage({ content, query }) {
  const section = content.technologyPage ?? defaultTechnologyPage
        ...
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            <header className="breaking-page__header">
              <h1>{section.title ?? 'टेक्नोलॉजी'}</h1>
            </header>
            ...
            <nav className="breaking-page__pagination" aria-label="Technology page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?technology=1&page=${pageNumber}`}
