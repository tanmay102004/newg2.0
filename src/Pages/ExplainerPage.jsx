...
function ExplainerPage({ content, query }) {
  const section = content.explainerPage ?? defaultExplainerPage
        ...
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            <header className="breaking-page__header">
              <h1>{section.title ?? 'एक्सप्लेनर'}</h1>
            </header>
            ...
            <nav className="breaking-page__pagination" aria-label="Explainer page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?explainer=1&page=${pageNumber}`}
