...
import { defaultDharmaCulturePage } from '../data/dharmaCulturePage'
...
function DharmaCulturePage({ content, query }) {
  const section = content.dharmaCulturePage ?? defaultDharmaCulturePage
        ...
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            <header className="breaking-page__header">
              <h1>{section.title ?? 'धर्म और संस्कृति'}</h1>
            </header>
            ...
            <nav className="breaking-page__pagination" aria-label="Dharma and culture page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?dharmaCulture=1&page=${pageNumber}`}
...
export default DharmaCulturePage
