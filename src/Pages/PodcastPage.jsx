...
import { defaultPodcastPage } from '../data/podcastPage'
...
function PodcastPage({ content, query }) {
  const section = content.podcastPage ?? defaultPodcastPage
      ...
      <div className="breaking-page">
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            ...
            <nav className="breaking-page__pagination" aria-label="Podcast page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?podcast=1&page=${pageNumber}`}
