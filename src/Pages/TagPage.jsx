...
import { getResolvedTagPageContent } from '../data/tagPages'
...
function TagPage({ content, query }) {
  const tagName = query.get('tag')?.trim() ?? ''
  const section = getResolvedTagPageContent(tagName, content)
      ...
      <div className="breaking-page">
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
                  ...
                  <article key={story.id} className="breaking-page__card">
                    <a className="breaking-page__card-link" href={href} {...linkBehavior}>
                      ...
                      <div className="breaking-page__meta">
                        <span>
                          <UserIcon />
                          <a className="breaking-page__author-link" href={buildAuthorHref(story.author)}>
                            {story.author}
                          </a>
                        </span>
                        <span>
            ...
            <nav className="breaking-page__pagination" aria-label="Tag page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?tag=${encodeURIComponent(tagName)}&page=${pageNumber}`}
