...
import { defaultNewsKhidkiPage } from '../data/newsKhidkiPage'
...
function NewsKhidkiPage({ content, query }) {
  const section = content.newsKhidkiPage ?? defaultNewsKhidkiPage
        ...
        <div className="breaking-page__layout">
          <section className="breaking-page__main">
            <header className="breaking-page__header">
              <h1>{section.title ?? 'न्यूज़ खिड़की'}</h1>
            </header>
            ...
            <nav className="breaking-page__pagination" aria-label="News Khidki page navigation">
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
                <a
                  key={pageNumber}
                  href={`/?newsKhidki=1&page=${pageNumber}`}
